setwd("D:/R/Hot_tumor_and_cold_tumor/immune data1/DGEs/cor/GSVA2")


cancers <- c("BLCA","CESC","PAAD","SARC","SKCM")

data <-  fread(paste("D:/R/Hot_tumor_and_cold_tumor/immune data1/DGEs/cor/GSVA/data.p_BLCA.csv",sep=''))
...
for(i in 1:length(pathyways)){
  ...
  for(cancer in cancers){
    ...
    datap <-  fread(paste("D:/R/Hot_tumor_and_cold_tumor/immune data1/DGEs/cor/GSVA/data.p_",cancer,".csv",sep=''))
    datap <- as.data.frame(datap)
    
    rownames(datap) <- datap[,1]
    datap <- datap[,-1]
    
    datar <-  fread(paste("D:/R/Hot_tumor_and_cold_tumor/immune data1/DGEs/cor/GSVA/data.r_",cancer,".csv",sep=''))
  ...
  myBreaks <- c(seq(-0.25, 0, length.out=ceiling(paletteLength/2) + 1), 
                seq(0.4/paletteLength, 0.55, length.out=floor(paletteLength/2)))
